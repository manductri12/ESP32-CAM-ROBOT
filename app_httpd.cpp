...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    else if(!strcmp(variable, "servoTilt")) // 3250, 4875, 6500
    {
      if      (val > 650) val = 650; //650
      else if (val < 225) val = 325; //325      
      ledcWrite(9,10*val);
      ...
      if (val==1) {
        ...
        Serial.println("NC_OFF");
        ledcWrite(14,0);       
        delay(100);
      }
      else if (val==7) {
        Serial.println("NC_ON");
        ledcWrite(14,255); 
        delay(100);
      }
      if (noStop!=1) 
...
<html>
    <head>
        ...
        <title>NGHIEN CUU KHOA HOC</title>
        <style>
            *{
                padding: 0; margin: 0;
                font-family:serif;
...
<table id="mainTable" style="width:650px;margin:auto;table-layout:fixed" CELLSPACING=30>
  <td>
        <h2 class="tITULO">TRƯỜNG ĐẠI HỌC GIAO THÔNG VẬN TẢI</h2>
        <h2 class="tITULO">PHÂN HIỆU TẠI TP.HCM</h2>
        <h2 class="tITULO1">ĐỒ ÁN CHUYÊN NGÀNH 1 &emsp; | &emsp; GVHD: LÂM QUANG THÁI</h2>
        ...
        <div class="cont_flex">     
            <button type="button" id="NC" onclick="fetch(document.location.origin+'/control?var=car&val=6');">NC OFF</button>
            <button type="button" id="NC" onclick="fetch(document.location.origin+'/control?var=car&val=7');">NC ON</button>  
        </div>
        <div class="cont_flex">  
            <div style="display: flex;align-items: center;">SVPan. <input type="range" id="servoPan" min="325" max="650" value="487" onchange="try{fetch(document.location.origin+'/control?var=servoPan&val='+this.value);}catch(e){}"></div>
            <div style="display: flex;align-items: center;">SVTil. <input type="range" id="servoTilt" min="325" max="650" value="487" onchange="try{fetch(document.location.origin+'/control?var=servoTilt&val='+this.value);}catch(e){}"></div>
        </div>
        <div class="cont_flex"> 
